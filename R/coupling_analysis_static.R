...
# add node roles (Guimera-Amaral) right after clustering
graph_with_roles <- add_node_roles(
  graph_with_cluster,
...
#' `saveRDS(graph_with_roles, here(clean_data_path, "graph_with_roles"))`
#' `graph_with_roles <- readRDS(here(clean_data_path, "graph_with_roles.rds"))`
...
# same graph but colored by field
...
#       field
#     ),
#     field_color = field_palette[field]
#   )

# graph_with_field <- graph_with_color %>%
#   activate(nodes) %>%
#   left_join(
#     nodes_field %>% select(source_id, field, field_color),
#     by = "source_id"
#   )

# labels_xy_field <- graph_with_field %>%
#   activate(nodes) %>%
#   as_tibble() %>%
#   group_by(dynamic_cluster_leiden) %>%
#   summarise(
...
#     field = names(sort(table(field), decreasing = TRUE))[1],
#     .groups = "drop"
#   ) %>%
#   filter(cluster_label != "Varia") %>%
#   mutate(field_color = field_palette[field])

# gg_field <- ggraph(graph_with_field, "manual", x = x, y = y) +
#   geom_edge_arc0(
#     aes(width = weight),
#     color = "grey80",
#     alpha = 0.4,
...
# gg_label_field <- gg_hull_field +
#   geom_label_repel(
#     data = labels_xy_field,
#     aes(
#       x = label_x,
#       y = label_y,
#       fill = field_color,
...
# ggsave(
#   paste0("coupling_static_field.jpg"),
#   device = "jpg",
#   plot = gg_label_field,
