...
saveRDS(
  summary,
  here(clean_data_path, paste0("summary_", size_time_window, ".rds"))
)
...
for (i in 1:length(dynamic_graph)) {
  ...
  if (i == 1) {
    ...
    past_position <- graphs_with_layout[[i - 1]] %>%
      activate(nodes) %>%
      as.data.table()
...
saveRDS(
  graphs_with_layout,
  here(clean_data_path, paste0("graphs_with_layout_", size_time_window, ".rds"))
)
...
# compute clusters
graphs_with_cluster <- add_clusters(
  graphs_with_layout,
...
graphs_with_cluster <- networkflow::merge_dynamic_clusters(
  graphs_with_cluster,
...
# save clustered graphs before roles
saveRDS(
  graphs_with_cluster,
  here(clean_data_path, paste0("graphs_with_cluster", size_time_window, ".rds"))
)
...
graphs_with_color <- networkflow::color_networks(
  graphs_with_cluster,
...
saveRDS(
  graphs_with_color,
  here(clean_data_path, paste0("graphs_with_color_", size_time_window, ".rds"))
)
#' `graphs_with_color <- readRDS(here(clean_data_path, paste0("graphs_with_color_", size_time_window, ".rds")))`

# add node roles (Guimera-Amaral) to each graph after clustering/merging
graphs_with_roles <- lapply(
  graphs_with_color,
  add_node_roles,
...
saveRDS(
  graphs_with_roles,
  here(clean_data_path, paste0("graphs_with_roles_", size_time_window, ".rds"))
)

#' `graphs_with_roles <- readRDS(here(clean_data_path, paste0("graphs_with_roles_", size_time_window, ".rds")))`

# ---------------------------- plot the graphs ---------------------------

list_dynamic_clusters <- graphs_with_roles %>%
  map(
    ~ .x %>%
      activate(nodes) %>%
      as_tibble() %>%
...
for (i in 1:length(graphs_with_roles)) {
  time_window <- names(graphs_with_roles)[[i]]

  cat(paste0("Plotting graph for the time window:", time_window), fill = TRUE)

  labels <- list_dynamic_clusters %>%
    mutate(
      dynamic_cluster_leiden_label = ifelse(
        is.na(label),
  ...
  gg <- ggraph(graphs_with_roles[[i]], "manual", x = x, y = y) +
    geom_edge_arc0(
  ...
  list_ggraph[[as.character(time_window)]] <- girafe(
    ggobj = gg,
    width_svg = width,
    height_svg = height
  )
...
saveRDS(
  list_ggraph,
  here(app_path, paste0("list_ggraph_", size_time_window, ".rds"))
)

# corpus
corpus <- graphs_with_roles %>%
  map(
    ~ .x %>%
      activate(nodes) %>%
      as_tibble()
  ...
  bind_rows() %>%
  mutate(
    first_year = str_extract(time_window, "^\\d{4}"),
...
# ----------------------------------- analysis of 4 main graphs ---------------------------- #

graphs_with_roles <- readRDS(here(
  clean_data_path,
  paste0("graphs_with_roles_", size_time_window, ".rds")
))


time_windows <- c("1971-1985", "1979-1993", "1988-2002", "1996-2010")

# filter the list of graphs

selected_graphs <- graphs_with_roles[
  names(graphs_with_roles) %in% time_windows
]
...
# remove colors columns in case of previous coloring
...
selected_graphs <- networkflow::color_networks(
  selected_graphs,
  column_to_color = "dynamic_cluster_leiden",
  color = df_color
)
...
for (i in 1:length(selected_graphs)) {
  ...
  data <- graph %>%
    ...
    left_join(
      tribble_label,
      by = c("dynamic_cluster_leiden" = "dynamic_cluster_leiden")
    )
  ...
  summary_table <- data %>%
    ...
    summarise(
      ...
      share_central = mean(is_central, na.rm = TRUE),
      share_connector = mean(is_connector, na.rm = TRUE)
    ) %>%
  ...
  saveRDS(
    summary_table,
    here(clean_data_path, paste0("summary_table_dynamic_", window, ".rds"))
  )
}
...
for (i in 1:length(selected_graphs)) {
  ...
  list_ggirafe[[as.character(time_window)]] <- girafe(
    ggobj = gg,
    width_svg = width,
    height_svg = height
  )
...
saveRDS(
  list_ggirafe,
  here(app_path, paste0("selected_graphs_", size_time_window, ".rds"))
)
